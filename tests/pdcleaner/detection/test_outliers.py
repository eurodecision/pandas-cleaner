...
def test_parameters_modify(anscombe):
    df = subset(anscombe, 'I')
    detector = df.cleaner.detect.outliers(eps=0.1, min_samples=3)
...
def test_type_error_parameter_eps(anscombe):
    ...
    with pytest.raises(TypeError, match=match):
        df.cleaner.detect.outliers(eps='string')
...
def test_type_error_parameter_min_samples(anscombe):
    ...
    with pytest.raises(TypeError, match=match):
        df.cleaner.detect.outliers(min_samples=2.5)
...
def test_dbscan_error():
    ...
    with pytest.raises(ValueError, match=match):
        dataframe.cleaner.detect('outliers').index
