"""Module to test plotting method associated with a value freq detector
"""
...
def test_plot_freq_nfirst_not_int(cat_series_with_nan):
    match = "should be an integer"
    detector = cat_series_with_nan.cleaner.detect.freq(freq=0.15)
    with pytest.raises(TypeError, match=match):
        detector.plot(nfirst='one')


def test_plot_freq_nlast_not_int(cat_series_with_nan):
    match = "should be an integer"
    detector = cat_series_with_nan.cleaner.detect.freq(freq=0.15)
    with pytest.raises(TypeError, match=match):
...
def test_plot_freq_nfirst_negative(cat_series_with_nan):
    match = "should be >=0"
    detector = cat_series_with_nan.cleaner.detect.freq(freq=0.15)
...
def test_plot_freq_nlast_negative(cat_series_with_nan):
    match = "should be >=0"
    detector = cat_series_with_nan.cleaner.detect.freq(freq=0.15)
    with pytest.raises(ValueError, match=match):
...
def test_plot_freq_no_arg(cat_series_with_nan):
    detector = \
        cat_series_with_nan.cleaner.detect('freq', freq=0.15)
    ax = detector.plot()
...
def test_plot_freq_nfirst_and_nlast_zeros(cat_series_with_nan):
    detector = \
        cat_series_with_nan.cleaner.detect('freq', freq=0.15)
...
def test_plot_freq_nfirst_only(cat_series_with_nan):
    detector = \
        cat_series_with_nan.cleaner.detect('freq', freq=0.15)
...
def test_plot_freq_nlast_only(cat_series_with_nan):
    detector = \
        cat_series_with_nan.cleaner.detect('freq', freq=0.15)
...
def test_plot_freq_nfirst_and_nlast(cat_series_with_nan):
    detector = \
        cat_series_with_nan.cleaner.detect('freq', freq=0.15)
...
def test_plot_freq_nfirst_and_nlast_sup_len_df(cat_series_with_nan):
    detector = \
        cat_series_with_nan.cleaner.detect('freq', freq=0.15)
